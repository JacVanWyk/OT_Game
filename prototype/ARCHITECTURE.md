...
- Load order in `index.html`: `js/assets_manifest.js` → `js/config.js` → `js/board.js` → `js/sprites.js` → `js/assets.js` → `js/game.js` (manifest first since v0.2.0; it only defines `OT.AM`).
...
- `window.__ready = true` is set synchronously once `game.js` has initialised (canvas sized, state = 'splash'). `window.__assetsReady = true` when the font AND every manifest image resolved (either outcome each).
...
## `js/assets_manifest.js` — `OT.AM` (GENERATED by `tools/preprocess_assets.py`, v0.2.0)

`OT.AM = { <fruit type id>: { src: 'assets/img/<Stem>.png', w, h }, … }`, keyed by the fruit **type id** (`'apple'`, not `'Apple'`) so the painter can look a tile's image up directly by `cell.type`. Only fruits that have a master render in `prototype/assets/<Stem>.png` appear; the preprocessor alpha-crops (alpha > 16) and downsizes each to 256 px max into `assets/img/`. As of 2026-09-03 this is **all 10 fruits of the roster** (bridge item J-001 closed), so the procedural painters in `js/sprites.js` are the loading/failure fallback only. `assets/Coconut.png` is in the repo but **deliberately absent from `FRUITS`** — Ben's decision (bridge MSG-05): the coconut is the tougher-tile mechanic (`{kind:'coconut'}`, never matchable), not an 11th fruit, so it must never reach `OT.AM` or `OT.S.fruit`. `tests/headless_smoke.mjs` asserts that, and doubles it as the negative control proving the differential probe can report zero.

## `js/assets.js` — font + real-art loader (port from Numbat Patrol `assets.js`)

1. Load Fredoka via `FontFace` from `(OT.AM_DATA && OT.AM_DATA['Fredoka-Bold']) || 'assets/fonts/Fredoka-Bold.woff2'` and add it to `document.fonts`.
2. Preload every `OT.AM` image (`OT.AM_DATA[typeId]` data URI from the bundle first, else the relative `src`). Snapshot `OT.S._proc.fruit` BEFORE any override. When **every** image has loaded, install an image-drawing `OT.S.fruit` in one atomic step (`OT.A.status = 'ready'`); if **any** fails, leave the procedural painters untouched (`'failed'`) — never a half-real set. The override draws the image for a type that has one (longer side = `0.95 × size`, same radial drop shadow and idle wobble as the procedural painter) and **delegates** to `OT.S._proc.fruit` for types without one, so a Summer+ board mixes real and procedural fruit by design until the roster is complete.
3. `window.__assetsReady = true` only when both the font and the images have resolved (either outcome each).

Expose `OT.A = { fontReady, fontError, status:'loading'|'ready'|'failed', total, loaded, failed:[], get(typeId) }`, `OT.S._proc = { fruit }`, and `OT.S.fruitImages` (type ids, only when ready). Test invariant: `'ready'` ⇒ `OT.S.fruit !== OT.S._proc.fruit`; `'failed'` ⇒ they are the same function.
...
Port of Numbat Patrol's: inline the six scripts in order, embed `Fredoka-Bold.woff2` as `OT.AM_DATA['Fredoka-Bold']` and every `OT.AM` image as `OT.AM_DATA[typeId]` (base64 data URIs, parsed out of `js/assets_manifest.js`) injected before the first script, write `dist/OrchardToss.html`. Fail loudly if any script anchor is missing/duplicated/out of order, if the font or any manifest image is missing / empty / not a PNG, if the embedded data-URI count differs from the manifest, or if any external `src=`/`href=` survives. Print the size.
...
- `tests/headless_smoke.mjs` — drives `~/.claude/tools/browser-harness/run.mjs` against a local `http.server` (ES-module harness needs an HTTP origin; the game itself is file-safe) and against `dist/OrchardToss.html` on `file://`: boots to `__ready`, `__assetsReady`, title → playing via `OT.game.start()`, `OT.debug.launch()` + `resolve()` changes `remaining`, timer decreases with `step`, `skipTo(52)` boots the last level, `failLevel()` drops a heart, a screenshot of playing at 390×844 and 844×390, a rendered-pixel probe that the board region is not the background colour, and (v0.2.0) a real-art differential probe: every imaged fruit rendered through `OT.S.fruit` must differ from its `OT.S._proc.fruit` render, and non-imaged fruits must render pixel-identical (negative control, with a non-trivial opaque count so an empty render cannot pass); both the http boot and the `file://` bundle boot require `OT.A.status === 'ready'` with all images loaded. Prints PASS/FAIL per check and exits non-zero on any failure.
