...
`OT.AM = { <fruit type id>: { src: 'assets/img/<Stem>.png', w, h }, … }`, keyed by the fruit **type id** (`'apple'`, not `'Apple'`) so the painter can look a tile's image up directly by `cell.type`. Only fruits that have a master render in `prototype/assets/<Stem>.png` appear; the preprocessor alpha-crops (alpha > 16) and downsizes each to 256 px max into `assets/img/`. Currently apple, banana, cherry, pineapple, strawberry; add a stem → type id to `FRUITS` in the tool when Ben supplies the rest.
